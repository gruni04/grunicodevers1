...
class OurPodcastController extends Controller
{
    public function index(Request $request)
    {
        $data['title']="Our Podcast";
        $data['title_url']=route('web-setting.our-podcast');
        $data['sub_title']="Our Podcast List";
        return view('admin.web-setting.our-podcast.index',$data);
    }
    ...
    public function save($id='')
    {
        $data['title']="Our Podcast";
        $data['title_url']=route('web-setting.our-podcast');
        $data['sub_title']="Save Our Podcast";
        $data['edit_data'] = array();
        if(!empty($id)){
            $data['edit_data'] = DB::table('tbl_our_podcast')->where('id', $id)->first();
        }

        return view('admin.web-setting.our-podcast.save', $data);
    }
    ...
    public function datatables(Request $request){
        ...
        // Total records
        $totalRecords = DB::table('tbl_our_podcast')->select('count(*) as allcount')->count();

        $totalRecordswithFilter = DB::table('tbl_our_podcast')->select('count(*) as allcount')->where('title', 'like', '%' .$searchValue . '%')->count();

        // Fetch records
        $records = DB::table('tbl_our_podcast')->orderBy($columnName, $columnSortOrder)
        ...
        foreach($records as $val){
            $arr["id"] = $sr++;
            $arr["title"] = $val->title;
            $arr["image"] = '<img src="'.url('uploads/our-podcast/'.$val->image).'" class="img img-fluid">' ;
            $arr["link"] = $val->link;
    ...
    {
        if($id){
            // $banner = DB::table('tbl_our_podcast')->where('id', $id)->first();
            // $this->removeFile('uploads/our-podcast',$banner->image);
            DB::table('tbl_our_podcast')->where('id', $id)->delete();
            $success = true;
            $message = "Our Podcast deleted successfully";
        }else{
            $success = true;
            $message = "Our Podcast not found";
        }
        return redirect()->route('web-setting.our-podcast')->with('success', $message);
    }
}
