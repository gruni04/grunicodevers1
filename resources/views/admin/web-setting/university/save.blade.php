...
@section('content')
	...
	<div class="card">
	    <div class="card-header border bottom">
	        ...
	        <a href="{{ route('web-setting.university') }}" class="btn btn-gradient-success" style="float: right;">Back</a>
	    </div>
	    <div class="card-body">
	        <div class="row">
	            <div class="col-sm-12">
	            	
	            	<form action="{{ route('web-setting.store-university') }}" method="POST" class="m-t-15" id="form">
                        ...
                        <div class="form-row">
                        	<div class="col-md-6">
                                <div class="form-group">
                                	...
                                	<select name="category" class="form-control" required>
                                        ...
                                        @foreach($university_category as $k=>$v)
                                        <option value="{{ $v->slug }}" {{ !empty($edit_data) && $v->slug==$edit_data->category ? "selected":''}} >{{ $v->category }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                	...
                                	<img src="{{ $edit_data && $edit_data->image ? url('uploads/university/'.$edit_data->image) : ''  }}" class="img img-fluid" style="width: 25%;">
                                	@endif
                                </div>
                            </div>
		              ...
		              success:function(responce)
		              {
		                 if(responce.status==1)
		                {
		                    ...
		                    window.setTimeout(function() {
		                    	window.location.href = '{{ route('web-setting.university') }}';
		                    }, 1500);
		                }else{
