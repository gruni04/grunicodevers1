		...
		html += '<div class="col-md-12 form-row application-document-section row-section">\
	                <div class="col-md-2">\
	                    <div class="form-group">\
	                        ...
	                        <input type="text" value="" class="form-control attatchment_type" name="attatchment_type[]" placeholder="Document Type" list="attatchment_type">\
	                ...
	                <div class="col-md-2">\
	                    <div class="form-group">\
	                        ...
	                        <input type="text" value="" class="form-control attatchment_value" name="attatchment_value[]" placeholder="Enter Score/Marks">\
	                    </div>\
	                </div>\
	                <div class="col-md-3">\
	                    <div class="form-group">\
              ...
              success:function(responce)
              {
                if(responce.status==1)
                {
                    ...
                    if(responce.aaData.length>0){
                    	var html = '';
                    	var total_fee = {{ $edit_data && $edit_data->application_fee ? $edit_data->application_fee : 0  }};
                    	var remaining_fee = 0;
                    	var submited_fee = 0;
                    	var curr_symb = '{{ $edit_data && $edit_data->currency && $edit_data->currency==1 ? "₹" : ($edit_data && $edit_data->currency && $edit_data->currency==2 ? "$" : "")  }}';
                    	for (var i = 0; i < responce.aaData.length; i++) {
                    		var fee = responce.aaData[i];
                    		
                    		if(fee.approval_status=='1'){
                    		    submited_fee += fee.fee_amount;
                    		}
                    		html += '<tr>\
			                            <td>'+(i+1)+'</td>\
			                            <td>'+fee.fee_year+'</td>\
                    	...
                    	remaining_fee = total_fee-submited_fee;
                    	console.log("total_fee-remaining_fee", curr_symb + remaining_fee);
                    	$(".remaining-fee").html(curr_symb + remaining_fee);
                    }
                }else{
                   _error(responce.message); 
                }
